...
from app.services.features.persistence import save_salary_features
from app.services.features.salary_feature_builders import (
    build_salary_feature_rows,
)


async def run_salary_features_pipeline(
    session: AsyncSession,
    client_id: int,
    date_from: datetime,
    date_to: datetime,
) -> dict[str, int | str | bool]:
    """Run salary feature engineering pipeline.
    Args:
    ...
    """
    feature_run_name = build_feature_run_name(feature_group='salary_features')
    ...
    salary_feature_rows = build_salary_feature_rows(
        data=snapshot_data,
        feature_run_id=feature_run.id,
    )
    salary_features = await save_salary_features(
        session=session,
        salary_feature_rows=salary_feature_rows,
    )

    feature_run.feature_count = len(salary_features)
    await session.flush()
