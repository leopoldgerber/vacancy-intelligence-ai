...
from app.services.features.persistence import save_text_features
from app.services.features.text_feature_builders import (
    build_text_feature_rows,
)


async def run_text_features_pipeline(
    session: AsyncSession,
    client_id: int,
    date_from: datetime,
    date_to: datetime,
) -> dict[str, int | str | bool]:
    """Run text feature engineering pipeline.
    Args:
    ...
    """
    feature_run_name = build_feature_run_name(feature_group='text_features')
    ...
    text_feature_rows = build_text_feature_rows(
        data=snapshot_data,
        feature_run_id=feature_run.id,
    )
    text_features = await save_text_features(
        session=session,
        text_feature_rows=text_feature_rows,
    )

    feature_run.feature_count = len(text_features)
    await session.flush()
