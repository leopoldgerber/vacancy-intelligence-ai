...
from app.services.features.persistence import save_time_features
from app.services.features.time_feature_builders import (
    build_time_feature_rows,
)


async def run_time_features_pipeline(
    session: AsyncSession,
    client_id: int,
    date_from: datetime,
    date_to: datetime,
) -> dict[str, int | str | bool]:
    """Run time feature engineering pipeline.
    Args:
    ...
    time_feature_rows = build_time_feature_rows(
        data=snapshot_data,
        feature_run_id=feature_run.id,
    )
    time_features = await save_time_features(
        session=session,
        time_feature_rows=time_feature_rows,
    )

    feature_run.feature_count = len(time_features)
    await session.flush()
